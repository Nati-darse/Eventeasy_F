...
const PaymentService = {
  async verifyIdentity(email, password) {
    try {
      ...
      const response = await axios.post(
        `https://event-easy.onrender.com/Event-Easy/payment/verify-identity`,
        { email, password },
        {
          headers: {
  ...
  async initializePayment(eventId) {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post(
        `https://event-easy.onrender.com/Event-Easy/payment/initialize/${eventId}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  async getPaymentStatus(txRef) {
    try {
      ...
      const response = await axios.get(
        `https://event-easy.onrender.com/Event-Easy/payment/status/${txRef}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        ...
        if (isStatusPage) {
          ...
          if (paymentData.data.eventId) {
            const eventResponse = await axios.get(
              `https://event-easy.onrender.com/Event-Easy/Event/events/${paymentData.data.eventId}`,
              {
                headers: {
          ...
          // Fetch event details
          const eventResponse = await axios.get(
            `https://event-easy.onrender.com/Event-Easy/Event/events/${eventId}`,
            {
              headers: {
                Authorization: `Bearer ${localStorage.getItem('token')}`,
              },
